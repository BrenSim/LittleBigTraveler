    ...
    public class ServiceController : Controller
    {
        ...
        public IActionResult AddServices(ServiceViewModel model)
        {
            if (ModelState.IsValid)
            {
                using (var serviceDAL = new ServiceDAL())
                {
                    int serviceId = serviceDAL.CreateService(model.Name, model.Price, model.Schedule, model.Location, model.Type, model.Style, model.Images, model.ExternalLinks); // Ajout de model.DestinationId
        ...
        public IActionResult ChangeServices(int id, ServiceViewModel model)
        {
            if (ModelState.IsValid)
            {
                using (var serviceDAL = new ServiceDAL())
                {
                    serviceDAL.ModifyService(id, model.Name, model.Price, model.Schedule, model.Location, model.Type, model.Style, model.MaxCapacity, model.Images, model.ExternalLinks); // Ajout de model.DestinationId
                }
